...
| `insight_search`   | `insights:read`  | Full-text search over live insights by `rank DESC, updated_at DESC, id DESC`. Returns bounded `standard` or `brief` snippets and metadata; use `insight_get` for full content. Optional `exclude_ids` suppresses up to 100 surfaced UUIDs before ranking and limiting. `query_mode=all` requires all terms; `query_mode=web` supports `OR`, quoted phrases, and exclusions. `tag_mode=all\|any` controls tag matching. Modes default to `all`; opaque cursor continuation is optional and bound to the canonical exclusion set. |
